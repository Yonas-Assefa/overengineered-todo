...
export class AuthController {
  ...
  public register = catchAsync(async (req: Request, res: Response) => {
    ...
    const tokens = await this.tokenService.generateAccessAndRefreshToken(
      userEntity
    );
  ...
  public login = catchAsync(async (req: Request, res: Response) => {
    ...
    const userEntity = await this.authService.loginWithEmailAndPassword(
      username,
      password
    );
    const tokens = await this.tokenService.generateAccessAndRefreshToken(
  ...
  public refreshToken = catchAsync(async (req: Request, res: Response) => {
    const { user: userEntity, tokens } = await this.authService.refreshAuth(
